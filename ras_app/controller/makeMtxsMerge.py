...
if __name__ == '__main__':
	...
	loaded = dict()
	loadmat(infn, loaded)
	nCases = loaded['Cli'].shape[0]
	for i in range(nCases):
		Cli.append(loaded['Cli'][i])
		RTm.append(loaded['RTm'][i])
		RTmCI.append(loaded['RTmCI'][i])
		Tm.append(loaded['Tm'][i])
		TmCI.append(loaded['TmCI'][i])
		NC.append(loaded['NC'][i])
		NT.append(loaded['NT'][i])
	# read fresh data from stdin
	record = False
