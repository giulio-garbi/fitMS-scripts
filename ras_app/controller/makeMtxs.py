...
if __name__ == '__main__':
	...
	NC = []
	NT = []
	entryNames = np.array(["Client", "persistence", "auth", "image", "webui"], dtype=object)
	...
	for line in sys.stdin:
		if line.startswith("True"):
			...
			RTm.append([0.0]*5)
			RTmCI.append([[0.0, 0.0]]*5)
			Tm.append([0.0]*5)
			TmCI.append([[0.0, 0.0]]*5)
			...
			if parts[0] == 'rt':
				if parts[1].startswith("Client"):
					...
					RTm[-1][4] = float(parts[2])
					RTmCI[-1][4] = [float(parts[3][1:-1]), float(parts[4][:-1])]
			elif parts[0] == 'thr':
				if parts[1].startswith("Client"):
					...
					Tm[-1][4] = float(parts[2])
					TmCI[-1][4] = [float(parts[3][1:-1]), float(parts[4][:-1])]
